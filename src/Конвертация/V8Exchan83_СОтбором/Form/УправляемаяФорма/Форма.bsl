...
Процедура ИзменитьДеревоПравилВыгрузки(СтрокиИсходногоДерева, СтрокиЗаменяемогоДерева)
	...
	КолонкаИспользоватьОтборы = СтрокиЗаменяемогоДерева.ВыгрузитьКолонку("ИспользоватьОтбор");
	СтрокиИсходногоДерева.ЗагрузитьКолонку(КолонкаИспользоватьОтборы, "ИспользоватьОтбор");
	...
	Для Каждого СтрокаИсходногоДерева Из СтрокиИсходногоДерева Цикл
		...
		Если НЕ ПустаяСтрока(СтрокаИзменяемогоДерева.АдресНастроекКомпоновщика) Тогда
			СтрокаИсходногоДерева.НастройкиКомпоновщика = ПолучитьИзВременногоХранилища(СтрокаИзменяемогоДерева.АдресНастроекКомпоновщика);
			СтрокаИсходногоДерева.СхемаКомпоновкиДанных = ПолучитьИзВременногоХранилища(СтрокаИзменяемогоДерева.АдресСКД);
			СтрокаИсходногоДерева.ВыбиратьДанныеДляВыгрузкиОднимЗапросом = ИСТИНА;
		КонецЕсли;
...
Процедура НастроитьКомпоновщик()
	...
	Если ТекущееПВД = Неопределено
		...
		Объект.КомпоновщикНастроек = Неопределено;
		
	Иначе
		Объект.КомпоновщикНастроек = Неопределено;
		
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("ИмяОбъектаДляЗапроса",      ТекущееПВД.ИмяОбъектаДляЗапроса);
		СтруктураПараметров.Вставить("Наименование",              ТекущееПВД.Имя);
		СтруктураПараметров.Вставить("ИспользоватьОтбор",         ТекущееПВД.ИспользоватьОтбор);
		СтруктураПараметров.Вставить("АдресНастроекКомпоновщика", ТекущееПВД.АдресНастроекКомпоновщика);
		СтруктураПараметров.Вставить("ИмяОбъектаДляЗапроса",      ТекущееПВД.ИмяОбъектаДляЗапроса);
		СтруктураПараметров.Вставить("АдресСКД",                  ТекущееПВД.АдресСКД);
		
		ТекущееПВД.АдресСКД = НастроитьКомпоновщикНаСервере(СтруктураПараметров, ДоступностьПостроителя);
		
	КонецЕсли;
	
	Элементы.КомпоновщикНастроекНастройкиОтбор.Доступность = ДоступностьПостроителя;
	
КонецПроцедуры

&НаСервере
Функция НастроитьКомпоновщикНаСервере(Правило, ДоступностьПостроителя)
	
	ИмяОбъекта                = Правило.Наименование;
	ТипОбъекта                = Правило.ИмяОбъектаДляЗапроса;
	АдресНастроекКомпоновщика = Правило.АдресНастроекКомпоновщика;
	ИмяОбъектаДляЗапроса      = Правило.ИмяОбъектаДляЗапроса;
	АдресСКД                  = Правило.АдресСКД;
	
	ИнициализироватьСКД    = Истина;
	ЭтоПерваяИнициализация = Ложь;
	...
	ИначеЕсли Найти(ТипОбъекта, "ПланВидовРасчета") > 0 Тогда
		ТипОбъекта = "ПланыВидовРасчета";
	ИначеЕсли Найти(ИмяОбъекта, "БизнесПроцесс") > 0 Тогда
		ТипОбъекта = "БизнесПроцессы";
	ИначеЕсли Найти(ТипОбъекта, "ПланВидовХарактеристик") > 0 Тогда
		ТипОбъекта = "ПланыВидовХарактеристик";
	ИначеЕсли Найти(ТипОбъекта, "РегистрСведений") > 0 Тогда	
		ТипОбъекта = "РегистрыСведений";
	Иначе
		ИнициализироватьСКД = Ложь;
	КонецЕсли;
	
	Если ИнициализироватьСКД Тогда 
		Если ПустаяСтрока(АдресСКД) Тогда
			...
			Для Каждого ОбъектМетаданных Из Метаданные[ТипОбъекта][ИмяОбъекта].СтандартныеРеквизиты Цикл
				НовоеПолеНабораДанных = НовыйНаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
				НовоеПолеНабораДанных.Заголовок = ОбъектМетаданных.Синоним;
				НовоеПолеНабораДанных.Поле = ОбъектМетаданных.Имя;
				НовоеПолеНабораДанных.ТипЗначения = ОбъектМетаданных.Тип;
				НовоеПолеНабораДанных.ПутьКДанным = ОбъектМетаданных.Имя;
			КонецЦикла;
			
			Для Каждого ОбъектМетаданных Из Метаданные[ТипОбъекта][ИмяОбъекта].Реквизиты Цикл
				НовоеПолеНабораДанных = НовыйНаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
				НовоеПолеНабораДанных.Заголовок = ОбъектМетаданных.Синоним;
				НовоеПолеНабораДанных.Поле = ОбъектМетаданных.Имя;
				НовоеПолеНабораДанных.ТипЗначения = ОбъектМетаданных.Тип;
			...
			Для Каждого ЭлементРеквизит Из Метаданные[ТипОбъекта][ИмяОбъекта].СтандартныеРеквизиты Цикл
				...
				НовоеПолеКомпоновкиДанных.Поле = Новый ПолеКомпоновкиДанных(ЭлементРеквизит.Имя);
				ВыбранноеПоле = НоваяГруппировка.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
				ВыбранноеПоле.Использование = Истина;
				ВыбранноеПоле.Заголовок = ЭлементРеквизит.Синоним;
				ВыбранноеПоле.Поле = Новый ПолеКомпоновкиДанных(ЭлементРеквизит.Имя);
			КонецЦикла;
				
			Для Каждого ЭлементРеквизит Из Метаданные[ТипОбъекта][ИмяОбъекта].Реквизиты Цикл
				НовоеПолеКомпоновкиДанных = НоваяГруппировка.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
				НовоеПолеКомпоновкиДанных.Использование = Истина;
				НовоеПолеКомпоновкиДанных.Поле = Новый ПолеКомпоновкиДанных(ЭлементРеквизит.Имя);
				ВыбранноеПоле = НоваяГруппировка.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		...
		Если ЭтоПерваяИнициализация Тогда
			Объект.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиКомпоновки);
		КонецЕсли;

		Если НЕ ПустаяСтрока(АдресНастроекКомпоновщика) Тогда
			Объект.КомпоновщикНастроек.ЗагрузитьНастройки(ПолучитьИзВременногоХранилища(АдресНастроекКомпоновщика));
		КонецЕсли;
...
&НаСервере
Процедура ЗаписатьНастройкиКомпоновщикаНаСервере(ИдентификаторСтроки)	
	
	ТекущееПВД = Объект.ТаблицаПравилВыгрузки.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	Если Объект.КомпоновщикНастроек.Настройки.Отбор.Элементы.Количество() > 0 Тогда
		
		ТекущееПВД.АдресНастроекКомпоновщика = ПоместитьВоВременноеХранилище(Объект.КомпоновщикНастроек.ПолучитьНастройки(), УникальныйИдентификатор);
		ТекущееПВД.ИспользоватьОтбор = Истина;
		
	Иначе
		
		ТекущееПВД.АдресНастроекКомпоновщика = "";
...
Процедура КомпоновщикНастроекНастройкиОтборПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ЗаписатьНастройкиКомпоновщикаНаСервере(Элементы.ТаблицаПравилВыгрузки.ТекущаяСтрока);
КонецПроцедуры
...
&НаСервере
Процедура ПоказатьРезультатОтбораНаСервере(ИдентификаторСтроки)
	
	Правило = Объект.ТаблицаПравилВыгрузки.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	СхемаКомпоновкиДанных = ПолучитьИзВременногоХранилища(Правило.АдресСКД);	
	...
	Если СхемаКомпоновкиДанных <> Неопределено Тогда
		...
		Если Найти(Правило.ИмяОбъектаДляЗапроса, "Справочник") > 0 Тогда
			ТипОбъекта = "Справочники";
			МенеджерОбъекта = Справочники[Правило.Имя];
		ИначеЕсли Найти(Правило.ИмяОбъектаДляЗапроса, "Документ") > 0 Тогда
			ТипОбъекта = "Документы";
			МенеджерОбъекта = Документы[Правило.Имя];
		ИначеЕсли Найти(Правило.ИмяОбъектаДляЗапроса, "ПланВидовРасчета") > 0 Тогда
			ТипОбъекта = "ПланыВидовРасчета";
			МенеджерОбъекта = ПланыВидовРасчета[Правило.Имя];
		ИначеЕсли Найти(Правило.ИмяОбъектаДляЗапроса, "БизнесПроцесс") > 0 Тогда
			ТипОбъекта = "БизнесПроцессы";
			МенеджерОбъекта = БизнесПроцессы[Правило.Имя];
		ИначеЕсли Найти(Правило.ИмяОбъектаДляЗапроса, "ПланВидовХарактеристик") > 0 Тогда
			ТипОбъекта = "ПланыВидовХарактеристик";
			МенеджерОбъекта = ПланыВидовХарактеристик[Правило.Имя];
		ИначеЕсли Найти(Правило.ИмяОбъектаДляЗапроса, "РегистрСведений") > 0 Тогда	
			ТипОбъекта = "РегистрыСведений";
			МенеджерОбъекта = РегистрыСведений[Правило.Имя];
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(ТипОбъекта) Тогда
			
			Для Каждого ОбъектМетаданных Из Метаданные[ТипОбъекта][Правило.Имя].СтандартныеРеквизиты Цикл
				НоваяКолонка = ТаблицаРезультат.Колонки.Добавить(ОбъектМетаданных.Имя, ОбъектМетаданных.Тип, ОбъектМетаданных.Синоним);
			КонецЦикла;
			
			Для Каждого ОбъектМетаданных Из Метаданные[ТипОбъекта][Правило.Имя].Реквизиты Цикл
				НоваяКолонка = ТаблицаРезультат.Колонки.Добавить(ОбъектМетаданных.Имя, ОбъектМетаданных.Тип, ОбъектМетаданных.Синоним);
			КонецЦикла;
			
			ВыборкаЭлемента = МенеджерОбъекта.Выбрать();
			Пока ВыборкаЭлемента.Следующий() Цикл
				ЗаполнитьЗначенияСвойств(ТаблицаРезультат.Добавить(), ВыборкаЭлемента);
			КонецЦикла;
			
		КонецЕсли;	
		
		
		Если НЕ ПустаяСтрока(Правило.АдресНастроекКомпоновщика) Тогда
			НастройкиКомпоновщика = ПолучитьИзВременногоХранилища(Правило.АдресНастроекКомпоновщика);
			...
			ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, Новый Структура("Основной", ТаблицаРезультат));
		...
		ПостроительОтчета.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТаблицаРезультат);
		ПостроительОтчета.ЗаполнитьНастройки();
		ПостроительОтчета.Выполнить();
		ПостроительОтчета.Вывести(ТабличныйДокумент);
		
		ТабличныйДокумент.Показать("Результат отбора");
...
Процедура ПоказатьРезультатОтбора(Команда)
	...
	ЗаписатьНастройкиКомпоновщикаНаСервере(ТекущаяСтрока);
	ПоказатьРезультатОтбораНаСервере(ТекущаяСтрока);
КонецПроцедуры
