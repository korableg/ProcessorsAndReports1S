...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено)
	...
	Если Правило.СхемаКомпоновкиДанных <> Неопределено Тогда
		...
		Если НЕ ПустаяСтрока(ТипОбъекта) Тогда
			...
			МакетКомпоновки = КомпоновщикМакета.Выполнить(Правило.СхемаКомпоновкиДанных, Правило.НастройкиКомпоновщика, , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
			...
			ПостроительЗапроса.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТаблицаРезультат);
...
Процедура ИнициализацияТаблицыПравилВыгрузки()
	...
	ДобавитьНедостающиеКолонки(Колонки, "НастройкиКомпоновщика");
	ДобавитьНедостающиеКолонки(Колонки, "СхемаКомпоновкиДанных");
